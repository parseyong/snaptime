...
public class JwtExceptionHandlerFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            log.info("[JwtExceptionHandlerFilter] error nickname = DecodingException");
        } catch (MalformedJwtException e) { //jwt 형식이 잘못되었을 때 발생, 헤더,페이로드,서명 누락 또는 잘못된 형식.
            setErrorResponse(response, ExceptionCode.TOKEN_INVALID);
            log.info("[JwtExceptionHandlerFilter] error nickname = MalformedJwtException");
        } catch (IllegalArgumentException e) { // 잘못된 인수나 인수의 값이 올바르지 않을 때 발생
            setErrorResponse(response, ExceptionCode.TOKEN_INVALID);
            log.info("[JwtExceptionHandlerFilter] error nickname = IllegalArgumentException");
            ...
            log.info("[JwtExceptionHandlerFilter] error nickname = NullPointerException");
        }
    }
